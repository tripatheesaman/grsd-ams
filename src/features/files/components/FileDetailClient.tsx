...
export default function FileDetailClient({ fileId, initialTab = "preview" }: Props & { initialTab?: TabKey }) {
  ...
  const leaveColumns = useMemo(() => {
    ...
    const ordered = preferredOrder.filter((col) => existing.includes(col));
    const extra = existing.filter((col) => !preferredOrder.includes(col));
              ...
              {leave?.rows?.map((row, idx) => (
                <tr key={idx}>
                  {leaveColumns.map((c) => (
                    <td
                      key={c}
                      className={`border-b px-2 py-1 align-top ${c.endsWith("_dates") ? "max-w-[420px] whitespace-normal break-words" : ""}`}
                    >
                      {String(row[c] ?? "")}
                    </td>
                  ))}
