...
import { readAttendanceRuleConfig, type AttendanceRuleConfig } from "@/server/settings/attendanceRules";
...
type LeaveSummaryRow = {
  ...
  leave_month: string;
  total_leave_taken_days: number;
  personal_leave_dates: string;
  sick_leave_dates: string;
  casual_leave_dates: string;
  substitute_leave_dates: string;
  duty_leave_dates: string;
...
type LeaveComputation = {
  ...
  dutyLeaveDays: number;
  otherLeaveDays: number;
  oddShiftDays: number;
  personalLeaveDates: string[];
  sickLeaveDates: string[];
  casualLeaveDates: string[];
  substituteLeaveDates: string[];
  dutyLeaveDates: string[];
  otherLeaveDates: string[];
  absentDates: string[];
};
type AttendanceComputationOptions = Pick<AttendanceRuleConfig, "oddShiftInBeforeMins" | "oddShiftOutAfterMins">;

const LEAVE_CODES_BY_TYPE: Record<LeaveType, Set<string>> = {
  pl: new Set(["PL", "ALCL"]),
  sl: new Set(["SL", "SLC"]),
  cl: new Set(["CL", "CLC"]),
  subl: new Set(["SUBL"]),
...
function normalizeStatusCode(statusRaw: unknown): string {
  return String(statusRaw ?? "").trim().toUpperCase().replace(/\s+/g, "");
}
function splitStatusTokens(statusRaw: unknown): string[] {
  return normalizeStatusCode(statusRaw)
    .split("/")
...
function hasBothTimes(inTime: unknown, outTime: unknown): boolean {
  return hasTimeValue(inTime) && hasTimeValue(outTime);
}
function hasOnlyInTime(inTime: unknown, outTime: unknown): boolean {
  return hasTimeValue(inTime) && !hasTimeValue(outTime);
}
function isInTimeBetween23And05(inTime: unknown): boolean {
  if (!hasTimeValue(inTime)) return false;
  const mins = parseHmToMinutes(inTime);
  if (mins === null) return false;
  const hour = Math.floor(mins / 60);
  return hour >= 23 || hour < 5;
}
function isWeeklyStatusToken(statusToken: string): boolean {
  return /^(WO|HO(\(\d+\))?)\*?$/.test(statusToken);
}
function qualifiesWeeklyOrAsteriskAllowance(inTime: unknown, outTime: unknown): boolean {
  const inMins = parseHmToMinutes(inTime);
  const outMins = parseHmToMinutes(outTime);
  const hasIn = inMins !== null && hasTimeValue(inTime);
  const hasOut = outMins !== null && hasTimeValue(outTime);
  if (hasIn && hasOut) return true;
  if (hasIn && !hasOut) return inMins > 5 * 60 && inMins < 22 * 60;
  if (!hasIn && hasOut) return outMins > 16 * 60;
  return false;
}
function isOddShiftDay(inTime: unknown, outTime: unknown, options: AttendanceComputationOptions): boolean {
  if (!hasBothTimes(inTime, outTime)) return false;
  const inMins = parseHmToMinutes(inTime);
  const outMins = parseHmToMinutes(outTime);
  if (inMins === null || outMins === null) return false;
  const adjustedOutMins = outMins < inMins ? outMins + 24 * 60 : outMins;
  return inMins < options.oddShiftInBeforeMins || adjustedOutMins > options.oddShiftOutAfterMins;
}
function hasAnyLeaveCode(statusCode: string): boolean {
  return Object.values(LEAVE_CODES_BY_TYPE).some((set) => [...set].some((code) => statusCode.includes(code)));
}
function classifyLeave(statusCode: string): LeaveType | null {
  if ([...LEAVE_CODES_BY_TYPE.pl].some((code) => statusCode.includes(code))) return "pl";
  if ([...LEAVE_CODES_BY_TYPE.sl].some((code) => statusCode.includes(code))) return "sl";
  if ([...LEAVE_CODES_BY_TYPE.cl].some((code) => statusCode.includes(code))) return "cl";
  if ([...LEAVE_CODES_BY_TYPE.subl].some((code) => statusCode.includes(code))) return "subl";
  if ([...LEAVE_CODES_BY_TYPE.duty].some((code) => statusCode.includes(code))) return "duty";
  if ([...LEAVE_CODES_BY_TYPE.other].some((code) => statusCode.includes(code))) return "other";
  return null;
}
function computeLeaveSummary(records: AttendanceRow[], options: AttendanceComputationOptions): LeaveComputation {
  const presentStatuses = new Set(["P", "HP", "HP/HA"]);
  const absentStatuses = new Set(["A", "HA", "ABSC", "LUR"]);
  const presentDutyLikeStatuses = new Set(["FW", "PFW", "TR", "PTR", "LD", "MT"]);
  const out: LeaveComputation = {
    presentDays: 0,
    absentDays: 0,
    weeklyOffDays: 0,
    allowanceDays: 0,
    ...
    sickLeaveDates: [],
    casualLeaveDates: [],
    substituteLeaveDates: [],
    dutyLeaveDates: [],
    otherLeaveDates: [],
    absentDates: [],
  };
  const pushLeave = (leaveType: LeaveType | null, date: string) => {
    if (leaveType === "pl") {
      out.personalLeaveDays += 1;
      out.personalLeaveDates.push(date);
    } else if (leaveType === "sl") {
      out.sickLeaveDays += 1;
      out.sickLeaveDates.push(date);
    } else if (leaveType === "cl") {
      out.casualLeaveDays += 1;
      out.casualLeaveDates.push(date);
    } else if (leaveType === "subl") {
      out.substituteLeaveDays += 1;
      out.substituteLeaveDates.push(date);
    } else if (leaveType === "duty") {
      out.dutyLeaveDays += 1;
      out.dutyLeaveDates.push(date);
      out.otherLeaveDays += 1;
    } else {
      out.otherLeaveDays += 1;
      out.otherLeaveDates.push(date);
    }
  };
  ...
  for (const row of records) {
    ...
    const statusTokens = splitStatusTokens(row.Status);
    const hasWeekly = statusTokens.some((token) => isWeeklyStatusToken(token));
    const hasAsteriskAbsent = statusTokens.includes("A*");
    const hasPresentLike =
      statusTokens.some((token) => presentStatuses.has(token) || presentDutyLikeStatuses.has(token));
    const hasAbsent = statusTokens.some((token) => absentStatuses.has(token));
    const date = String(row.Date ?? "");
    const inTime = row.InTime;
    const outTime = row.OutTime;
    if (isOddShiftDay(inTime, outTime, options)) out.oddShiftDays += 1;
    ...
    if (hasAsteriskAbsent) {
      out.presentDays += 1;
      if (qualifiesWeeklyOrAsteriskAllowance(inTime, outTime)) out.allowanceDays += 1;
      continue;
    }
    if (hasWeekly) {
      out.presentDays += 1;
    ...
    // Unknown status fallback: classify as "other leave" so records are not silently dropped.
    out.otherLeaveDays += 1;
    out.otherLeaveDates.push(date);
  }
...
function calcLeaveTotals(records: AttendanceRow[], options: AttendanceComputationOptions) {
  const totals = computeLeaveSummary(records, options);
  return {
    presentDays: totals.presentDays,
    absentDays: totals.absentDays,
    weeklyOffDays: totals.weeklyOffDays,
    allowanceDays: totals.allowanceDays,
    sickLeaveDays: totals.sickLeaveDays,
    casualLeaveDays: totals.casualLeaveDays,
    personalLeaveDays: totals.personalLeaveDays,
    substituteLeaveDays: totals.substituteLeaveDays,
    dutyLeaveDays: totals.dutyLeaveDays,
    otherLeaveDays: totals.otherLeaveDays,
  };
}

type HrmsWorkbookOptions = {
  employeeSortMeta?: Map<string, { priority: number; staffid: string }>;
  rules: AttendanceComputationOptions;
};
...
async function hrmsWorkbookBuffer(inputRows: AttendanceRow[], options?: HrmsWorkbookOptions): Promise<Buffer> {
  ...
  for (const [empId, empRows] of employees) {
    ...
    const totals = calcLeaveTotals(empRows, options?.rules ?? { oddShiftInBeforeMins: 5 * 60 + 30, oddShiftOutAfterMins: 21 * 60 });
...
async function writeHrmsWorkbook(outputPath: string, rows: AttendanceRow[], rules: AttendanceComputationOptions): Promise<void> {
  await fs.mkdir(path.dirname(outputPath), { recursive: true });
  const buf = await hrmsWorkbookBuffer(rows, { rules });
...
export async function processAttendance(inputPath: string, outputPath: string) {
  try {
    ...
    const rules = await readAttendanceRuleConfig();
    await writeHrmsWorkbook(outputPath, rows, rules);
    return {
      success: true,
...
function detectLeaveMonthLabel(rows: AttendanceRow[], inputPath: string): string {
  const monthNames = [
    "January",
    "February",
    "March",
  ...
  if (isoDate) {
    const [year, month] = isoDate.split("-").map((p) => Number.parseInt(p, 10));
...
export async function leaveSummary(inputPath: string, departmentStaffIds: string[]) {
  ...
  const leaveMonth = detectLeaveMonthLabel(rows, inputPath);
  const allowed = new Set(departmentStaffIds.map((s) => String(s)));
  const leaveList: LeaveSummaryRow[] = [];
  const formatLeaveDay = (row: AttendanceRow) => {
    const dateText = String(row.Date ?? "").trim();
    const dayName = String(row.Day_Name ?? "").trim();
    if (!dateText && !dayName) return "";
    if (dayName && dateText && !dateText.toLowerCase().includes(dayName.toLowerCase())) return `${dateText} (${dayName})`;
    return dateText || dayName;
  };
  const formatLeaveDates = (values: string[]) => values.filter(Boolean).join(", ");
  for (const [employeeId, employeeRows] of grouped.entries()) {
    if (allowed.size > 0 && !allowed.has(employeeId)) {
      continue;
    }
    const first = employeeRows[0];
    const totals = computeLeaveSummary(employeeRows, rules);
    const leaveDatesByType: Record<LeaveType, string[]> = {
      pl: [],
      sl: [],
      cl: [],
      subl: [],
    ...
    for (const row of employeeRows) {
      const leaveType = splitStatusTokens(row.Status).map((token) => classifyLeave(token)).find((v) => v !== null);
      if (!leaveType) continue;
      leaveDatesByType[leaveType].push(formatLeaveDay(row));
    }
    ...
    leaveList.push({
      ...
      total_leave_taken_days:
        ...
        totals.dutyLeaveDays +
        totals.otherLeaveDays,
      personal_leave_dates: formatLeaveDates(leaveDatesByType.pl),
      sick_leave_dates: formatLeaveDates(leaveDatesByType.sl),
      casual_leave_dates: formatLeaveDates(leaveDatesByType.cl),
      substitute_leave_dates: formatLeaveDates(leaveDatesByType.subl),
      duty_leave_dates: formatLeaveDates(leaveDatesByType.duty),
      other_leave_dates: formatLeaveDates(leaveDatesByType.other),
    });
  }
...
export async function segregationSectionReports(inputPath: string, departmentId?: string): Promise<SegregationSectionWorkbook[]> {
  ...
  for (const { meta, rows: sectionData, employeeSortMeta } of sectionRows.values()) {
    ...
    const buf = await hrmsWorkbookBuffer(sectionData, { employeeSortMeta, rules });
...
async function fillTemplate(
  ...
  for (const { staff, empId } of targetStaff) {
    ...
    const summary = computeLeaveSummary(empRows, rules);
    const presentDays = summary.presentDays;
    const absentDays = summary.absentDays;
    const allowanceDays = summary.allowanceDays;
    const personalLeaveDays = summary.personalLeaveDays;
    const sickLeaveDays = summary.sickLeaveDays;
    const casualLeaveDays = summary.casualLeaveDays;
    const substituteLeaveDays = summary.substituteLeaveDays;
    const otherLeaveDays = summary.otherLeaveDays;
    const oddShiftDays = summary.oddShiftDays;

    const remarksParts: string[] = [];
    const pushRemarks = (label: string, dates: string[]) => {
      if (dates.length > 0) remarksParts.push(`${label} on ${dates.map((d) => dayOnly(d)).join(", ")}`);
    };
    pushRemarks("PL", summary.personalLeaveDates);
    pushRemarks("CL", summary.casualLeaveDates);
    pushRemarks("SL", summary.sickLeaveDates);
    pushRemarks("SUBSTITUTE", summary.substituteLeaveDates);
    pushRemarks("DUTY", summary.dutyLeaveDates);
    pushRemarks("Other", summary.otherLeaveDates);
    pushRemarks("Absent", summary.absentDates);
