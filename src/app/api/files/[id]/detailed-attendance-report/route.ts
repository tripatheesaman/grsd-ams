...
import { absoluteFromMedia, PROCESSED_ROOT } from "@/server/files";
import { detailedReport } from "@/server/attendance";
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const outputPath = path.join(PROCESSED_ROOT, `detailed_attendance_${id}_${Date.now()}.xlsx`);
  const templatePath = path.join(process.cwd(), "public", "detailed_attendance_template.xlsx");

  try {
    await detailedReport(inputPath, outputPath, templatePath, user.isSuperuser ? undefined : user.departmentId?.toString());
